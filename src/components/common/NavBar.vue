<template>
  <nav class="w-full h-20 bg-app-bg text-18 border-b app-border-gray">
    <div class="px-20 py-5">
      <div class="flex justify-between">
        <div class="flex items-center">
          <div v-on:click="$router.push('/')">
            <img width="270" height="42" src="/dip-logo.png" />
          </div>
        </div>

        <div class="flex items-center">
          <a href="#" class="pr-3 border-r app-border-gray">คำแนะนำการใช้งาน</a>
          <a href="#" class="px-3 border-r app-border-gray">แจ้งปัญหาการใช้งาน</a>
          <a href="#" class="px-3 border-r app-border-gray">ตัวช่วยสำหรับคนพิการ</a>
          <div class="flex items-center px-3">
            <div class="flex h-[24px] w-[24px] flex-shrink-0 items-center justify-center">
              <PhoneFillIcon />
            </div>
            <div class="app-text-red font-semibold">1368</div>
          </div>
          <div class="px-3">
            <div class="flex items-center gap-2">
              <div class="h-[24px] w-[24px] overflow-hidden rounded-full">
                <img src="/th-flag.png" class="h-full w-full object-cover" alt="TH logo" />
              </div>
              <div>ไทย</div>
            </div>
          </div>
          <div class="px-3">
            <div class="flex items-center gap-2">
              <div class="flex h-[24px] w-[24px] flex-shrink-0 items-center justify-center">
                <AccountCircleIcon />
              </div>
              <div>เข้าสู่ระบบ</div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </nav>
</template>

<script setup lang="ts">
import PhoneFillIcon from '@/assets/icons/phone-fill.svg'
import AccountCircleIcon from '@/assets/icons/account_circle.svg'
</script>
